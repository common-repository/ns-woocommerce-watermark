 ...
 *
 * TODO Search & Replace ww_ with your prefix
 * TODO Search & Replace Ww_ with your prefix
 * TODO Search & Replace 'ns-woocommerce-watermark' with your 'ns-woocommerce-watermark'
...
class Ww_Modules_Reviews {
	 ...
	 * @var string
	 */
	public static $api_url = 'https://revieworgapi.nsthemes.com/wp-json/ww/v1/review_action';
	...
	public static function init() {
		...
		add_action( 'wp_ajax_ww_review_action', array( __CLASS__, 'ajax_handler' ) );
	}
	...
	public static function installed_on() {
		$installed_on = get_option( 'ww_reviews_installed_on', false );

		if ( ! $installed_on ) {
			$installed_on = current_time( 'mysql' );
			update_option( 'ww_reviews_installed_on', $installed_on );
		}
	...
	public static function ajax_handler() {
		...
		if ( ! wp_verify_nonce( $_REQUEST['nonce'], 'ww_review_action' ) ) {
			wp_send_json_error();
		}
		...
		try {
			...
			update_user_meta( $user_id, '_ww_reviews_dismissed_triggers', $dismissed_triggers );
			update_user_meta( $user_id, '_ww_reviews_last_dismissed', current_time( 'mysql' ) );
	...
	public static function dismissed_triggers() {
		...
		$dismissed_triggers = get_user_meta( $user_id, '_ww_reviews_dismissed_triggers', true );
	...
	public static function already_did( $set = false ) {
		...
		if ( $set ) {
			update_user_meta( $user_id, '_ww_reviews_already_did', true );

			return true;
		}

		return (bool) get_user_meta( $user_id, '_ww_reviews_already_did', true );
	}
	...
	public static function triggers( $group = null, $code = null ) {
		...
		if ( ! isset( $triggers ) ) {
			...
			$open_message = __( 'Hey, I noticed you recently hit %d Watermark on your site – that’s awesome!. I would very much appreciate if you could quickly give it a 5-star rating on WordPress, just to help us spread the word.', 'ns-woocommerce-watermark' );
			$time_message = __( "Hey, you've been using Watermark for %s on your site - I hope that its been helpful. I would very much appreciate if you could quickly give it a 5-star rating on WordPress, just to help us spread the word.", 'ns-woocommerce-watermark' );

			$triggers = apply_filters( 'ww_reviews_triggers', array(
				'time_installed' => array(
					'triggers' => array(
						'one_week'     => array(
							'message'    => sprintf( $time_message, __( '1 week', 'ns-woocommerce-watermark' ) ),
							'conditions' => array(
								strtotime( self::installed_on() . ' +1 week' ) < time(),
							),
							'link'       => 'https://wordpress.org/support/plugin/ns-woocommerce-watermark/reviews/?rate=5#rate-response',
							'pri'        => 10,
						),
						'one_month'    => array(
							'message'    => sprintf( $time_message, __( '1 month', 'ns-woocommerce-watermark' ) ),
							'conditions' => array(
						...
						'three_months' => array(
							'message'    => sprintf( $time_message, __( '3 months', 'ns-woocommerce-watermark' ) ),
							'conditions' => array(
				...
				'open_count'     => array(
					'triggers' => array(
						'50_opens'  => array(
							...
							'conditions' => array(
								get_option( 'ww_total_open_count', 0 ) > 50,
							),
							'link'       => 'https://wordpress.org/support/plugin/ns-woocommerce-watermark/reviews/?rate=5#rate-response',
							'pri'        => 10,
						),
						'100_opens' => array(
							...
							'conditions' => array(
								get_option( 'ww_total_open_count', 0 ) > 100,
						...
						'500_opens' => array(
							...
							'conditions' => array(
								get_option( 'ww_total_open_count', 0 ) > 500,
							),
							'link'       => 'https://wordpress.org/support/plugin/ns-woocommerce-watermark/reviews/?rate=5#rate-response',
							'pri'        => 30,
						),
	...
	public static function admin_notices() {
			...
			(function ($) {
				...
				function dismiss(reason) {
					$.ajax({
						...
						url: ajaxurl,
						data: {
							action: 'ww_review_action',
							nonce: '<?php echo wp_create_nonce( 'ww_review_action' ); ?>',
							group: trigger.group,
							code: trigger.code,
							pri: trigger.pri,
							reason: reason
		...
		<div class="notice notice-success is-dismissible prefix-notice">
			...
			<p>
				<img class="logo" src="https://ps.w.org/ns-woocommerce-watermark/assets/icon-128x128.png" />
				<strong>
			...
			<ul>
				<li>
					<a class="prefix-dismiss" target="_blank" href="https://wordpress.org/support/plugin/ns-woocommerce-watermark/reviews/?rate=5#rate-response" data-reason="am_now">
						<strong><?php _e( 'Ok, you deserve it', 'ns-woocommerce-watermark' ); ?></strong>
					</a>
				</li>
				<li>
					<a href="#" class="prefix-dismiss" data-reason="maybe_later">
						<?php _e( 'Nope, maybe later', 'ns-woocommerce-watermark' ); ?>
					</a>
				</li>
				<li>
					<a href="#" class="prefix-dismiss" data-reason="already_did">
						<?php _e( 'I already did', 'ns-woocommerce-watermark' ); ?>
					</a>
				</li>
			</ul>
	...
	public static function last_dismissed() {
		...
		return get_user_meta( $user_id, '_ww_reviews_last_dismissed', true );
	}
